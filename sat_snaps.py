...
def safe_write_bytes(directory: Path, suffix: str, data: bytes,
    ...
    """
    Atomically write `data` to a temp file inside `directory` using O_CREAT|
    O_EXCL|O_NOFOLLOW (defeats symlink TOCTOU), chmod 0600, then optionally
    rename to `final_name`. Returns the final Path, or None on error.
    """

    fd, tmp_path = tempfile.mkstemp(suffix=suffix, dir=str(directory))
    try:
